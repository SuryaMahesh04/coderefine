...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    // Optimization: Clamping pixel ratio to 1.0 for background effects significantly reduces GPU load
    ...
    const uniforms = {
      iResolution: {
        value: new THREE.Vector3(container.clientWidth, container.clientHeight, 1.0)
      },
      iTime: { value: 0 },
    ...
    const onResize = () => {
      renderer.setSize(container.clientWidth, container.clientHeight);
      material.uniforms.iResolution.value.set(container.clientWidth, container.clientHeight, 1.0);
